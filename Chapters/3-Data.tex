...
esubsection{The Haversine Formula}
...
The python implementation of the formula can be seen in figure \ref{fig:haversine}. It is s imortant to note that the degrees taken as input
...
We can utilize this to get the distance in meters between the boundaries of our area of interest.
When all data is retrieved from wigle, we still have the specified coordinates that data was retrieved from.
Using these coordinates gives us the size of our cartesian coordinate system. First the distance between the first latitude $\varphi_1$ and the second latitude $\varphi_2$
is calculated, using the same longitude  $\lambda_1 = \lambda_2$. This returns the size of the x-axis in meters.
The opposite has to be done to get the size of the y-axis, using $\varphi_1 = \varphi_2$ and equal longitudes.
other axis. The same approach can be used to place each node on the coordinate system.
