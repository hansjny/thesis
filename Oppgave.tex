...
use the data for our group computation, we have to translate the global coordinates
to two dimensional coordinates on a plane. Euclidean geometry will then work,
and we can place each AP on a topology with the same format as the
generated topologies we created in section \ref{simdata}. This is necessary for two reasons.
...
The haversine formula gives us the distance in meters between to coordinates. By calculating
the distane between the latitude startpoint and the latitude endpoint, we can the size of one
axis. By computing the distance between the longitude start and endpoint we get the size of the
other axis. To place a node anywhere on this coordinate system, 
