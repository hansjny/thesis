...
\section{Network infrastructure}
		...
		authentication and association to all APs under the same extended service set. In an ESS the SSID of all APs are also identical. This means a STA can be in the basic service area of
...
\item DCF Mode IFS (PIFS), before RTS frames (or DCF data frames if RTS/CTS is disabled)
	...
	\section{PHY Layer}
	The physical layer in 801.11 is also divided in two sublayers. The upper sublayer is the Physical Layer Convergence Procedure (PLCP), responsible for clear channel assessment and acting
	...
	It is shown that the mentioned countermeasures only have limited
	impact, while switching to a channel with less interference (if available) remains the most effective.
	\cite{impactRF}. 
	\section{Channels} 
